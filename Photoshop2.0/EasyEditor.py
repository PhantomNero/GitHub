...
from PyQt5.QtCore import Qt  #библиотека для работы с размером фотографии
...
btn_dir.clicked.connect(showFilenamesList) #подключаем кнопку выбора директории

workimage = ImageProcessor()  # текущая рабочая картинка для работы
lw_files.currentRowChanged.connect(showChosenImage) #метод для отображения фото которое мы выбрали

btn_bw.clicked.connect(workimage.do_bw) #подключаем кнопку выбора режима ЧБ (чёрно-белый)
